...
Move karthy::GomokuAI::takeTurn(void)
{
	_game->writeText(Text::THINKING);
...
uint64_t karthy::GomokuAI::locateCurrentStateId(Move& adversaryMove, GomokuBoard& gameBoard)
{
	...
	else 
	{
		...
		for (forward_list<Action*>::iterator it = currentState->edgeList->begin(); it != currentState->edgeList->end(); ++it)
		{
			...
			this->myGame->board.setBoxStatus(logicalMove, (BoxStatus)!this->myPlayer);

			bool symetric = isSymmetric(this->myGame->board, gameBoard);

			this->myGame->board.setBoxStatus(logicalMove, BoxStatus::HAVE_NO_STONE);

			//check if got adversary's move
			if (symetric)
			{
				result = (*it)->otherNode->getId();
				this->myGame->executeMove(logicalMove);
				break;
			}
		}
	}
...
Action* karthy::GomokuAI::selectAction(DecisionTree& decisionTree)
{
	Action* action = NULL;
